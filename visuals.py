...
class Visualiser:
    ...
    def set_initial(self):
        project_name_line = f"Currently Tracking for: {self.styles.UNDERLINE}{self.styles.CYAN}{self._project}{self.styles.NORMAL}{self.styles.BOLD}"
        space_left = (self.FULL_WIDTH - (28 + len(self._project)))
        extra = int(not((space_left / 2) == (space_left // 2)))
        print(f"{self.styles.BOLD}{'='* self.FULL_WIDTH}")
        ...
        print(f"|| {self.styles.UNDERLINE}Session Aims{self.styles.NORMAL}{self.styles.BOLD}:{' ' * (self.AIMS_WIDTH-20)}||   || {self.styles.UNDERLINE}Commands{self.styles.NORMAL}{self.styles.BOLD}: {' ' * (self.HELP_WIDTH - 16)}||")
        for n, aim in enumerate(self._aims):
            distance_left = self.AIMS_WIDTH - (10 + len(aim))
            print(f"||{self.styles.NORMAL}{self.styles.WHITE} {n+1}. {aim}{' '*distance_left}{self.styles.NORMAL}{self.styles.BOLD}||   ||{' ' * (self.HELP_WIDTH - 5)}||")
        print("=" * (self.AIMS_WIDTH-2)," ","=" * ((self.FULL_WIDTH//4)-1))
